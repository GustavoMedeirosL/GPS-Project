...
class ORSGeocodingService:
    ...
    def geocode_with_state(self, address: str, timeout: int = 10):
        """
        Geocodifica um endereço e tenta extrair o estado (UF) do resultado.

        Utiliza a resposta GeoJSON do ORS para ler os campos `region` ou
        `region_a` de `features[0]['properties']`, que tipicamente contêm
        o nome do estado brasileiro.
        ...
        Returns:
            Tuple[float, float, Optional[str]]: (latitude, longitude, estado_uf_raw)
            `estado_uf_raw` é o texto bruto do estado retornado pelo geocoder —
            pode ser None se não disponível. A normalização para sigla é feita
            pelo fuel_service.normalize_state_to_uf().
        ...
        try:
            ...
            # Extrair estado dos metadados do geocoder
            props = feature.get('properties', {})
            # ORS retorna 'region' com o nome do estado para resultados no Brasil
            state_raw = props.get('region') or props.get('region_a') or None

            return (lat, lon, state_raw)
